...
<div class="page-heading">
  <div class="page-heading-copy">
    ...
    <div>
      ...
      <p class="text-muted mb-0">Track preparation, delivery status, and historical timeline for in-house guest breakfasts.</p>
    </div>
  </div>
</div>
  ...
  <form method="GET" action="{{ route('fb.breakfast') }}" class="row g-2 align-items-end">
    <div class="col-12 col-sm-6 col-md-4">
      ...
      <div class="input-group input-group-sm">
        <input type="date" name="date" id="date" value="{{ $date }}" class="form-control" style="border-radius: 8px 0 0 8px;">
        <button type="submit" class="btn btn-primary btn-tactile"><i class="bi bi-calendar-check"></i> Load Date</button>
      </div>
    </div>
  </form>
</div>
  ...
  <div class="table-responsive">
    <table class="table align-middle table-hover mb-0">
      ...
      <tbody>
          ...
          <tr>
            <td>
              <div class="d-flex align-items-center">
                <span class="avatar bg-primary-subtle text-primary fw-bold me-2 rounded-circle d-flex align-items-center justify-content-center" style="width: 38px; height: 38px; font-size: 0.85rem; border: 2px solid var(--admin-primary);">
                  {{ $res->room->room_number }}
                </span>
                <div>
                  <strong>Room {{ $res->room->room_number }}</strong>
                  <div class="text-xs text-muted" style="font-size: 0.72rem;">{{ $res->room->roomType->name }}</div>
                </div>
              </div>
            </td>
            <td>
              <div class="fw-bold text-body">{{ $res->guest->name }}</div>
              <div class="text-xs text-muted" style="font-size: 0.72rem;">Res No: <span class="font-monospace fw-semibold">{{ $res->reservation_number }}</span></div>
              <div class="text-xs text-muted mt-1" style="font-size: 0.72rem;"><i class="bi bi-people-fill text-secondary"></i> Capacity: <strong>{{ $br->pax }} Pax</strong></div>
            </td>
            <td>
              @if($res->room->roomType->breakfast_included)
                <span class="badge badge-soft-success"><i class="bi bi-check-circle-fill"></i> Complimentary</span>
              @else
                <span class="badge badge-soft-warning"><i class="bi bi-exclamation-triangle-fill"></i> Add-on Paid</span>
              @endif
            </td>
            <td>
              <div class="mb-2">
              ...
              @if($br->timeline && is_array($br->timeline))
                <div class="ps-1 small" style="font-size: 0.72rem;">
                  ...
                  @foreach($steps as $index => $step)
                    <div class="breakfast-timeline-item {{ $index === 0 ? 'active-step' : 'done-step' }}">
                  ...
                  @if(count($br->timeline) > 3)
                    <div class="text-xs text-primary cursor-pointer mt-1 fw-bold btn-tactile" data-bs-toggle="collapse" data-bs-target="#fullTimeline-{{ $br->id }}">
              ...
              @endif
              @if($br->notes)
                <div class="text-xs text-muted mt-2 italic" style="font-size: 0.72rem;"><i class="bi bi-chat-left-text text-secondary me-1"></i> "{{ $br->notes }}"</div>
              @endif
            </td>
            <td class="text-end">
              <div class="d-flex justify-content-end gap-1">
                @if($br->status === 'Pending')
                  <button type="button" class="btn btn-warning btn-tactile btn-xs py-1" onclick="openBreakfastModal('{{ $br->id }}', 'Preparing')">
                    <i class="bi bi-play-fill"></i> Prepare
                  </button>
                  <button type="button" class="btn btn-outline-danger btn-tactile btn-xs py-1" onclick="openBreakfastModal('{{ $br->id }}', 'Skipped')">
                    <i class="bi bi-x-lg"></i> Skip
                  </button>
                @elseif($br->status === 'Preparing')
                  <button type="button" class="btn btn-success btn-tactile btn-xs py-1" onclick="openBreakfastModal('{{ $br->id }}', 'Delivered')">
                ...
                @else
                  <button type="button" class="btn btn-outline-secondary btn-tactile btn-xs py-1" onclick="openBreakfastModal('{{ $br->id }}', 'Pending')">
        ...
        @empty
          <tr>
            <td colspan="5" class="text-center text-muted py-5" style="font-size: 0.85rem;">No active checked-in guests scheduled for breakfast entitlements on this date.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
...
<div class="modal fade" id="breakfastStatusModal" tabindex="-1" aria-labelledby="breakfastStatusModalLabel" aria-hidden="true">
    ...
    <div class="modal-content shadow-lg border-0" style="border-radius: 14px;">
      ...
      <form id="breakfastStatusForm" method="POST" action="">
        ...
        <div class="modal-body p-4">
          ...
          <div class="mb-0">
            <label for="breakfast_notes" class="form-label small fw-bold">Notes / Special Instructions (Optional)</label>
            <textarea name="notes" id="breakfast_notes" rows="2" class="form-control form-control-sm" placeholder="e.g. No onions, extra chili, or skip reason..." style="border-radius: 8px;"></textarea>
          </div>
        </div>
        <div class="modal-footer border-top">
          <button type="button" class="btn btn-light btn-sm btn-tactile" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary btn-sm btn-tactile" id="breakfast_submit_btn">Update Status</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  function openBreakfastModal(recordId, status) {
    ...
    confirmMsg.innerHTML = `Are you sure you want to update the breakfast status for this room to <strong>${status}</strong>? Please enter any special instructions or skip reason below:`;
