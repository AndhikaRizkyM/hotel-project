...
@section('content')
    <div class="page-heading">
        ...
        <div class="heading-actions">
            <a href="{{ route('fo.availability') }}" class="btn btn-light btn-tactile btn-sm px-3 py-2"><i
                    class="bi bi-arrow-left"></i> Back to Room Map</a>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('fo.reservations.store') }}" id="reservationForm">
        ...
        <div class="row g-4">
            ...
            <div class="col-12 col-lg-7">
                <div class="panel-premium shadow-sm h-100 p-4">
                    <div class="panel-header border-bottom pb-2 mb-3">
                        <h2 class="h5 mb-0 section-title"><i class="bi bi-person-fill text-primary"></i><span>Guest Identity
                                Details</span></h2>
                    </div>
                    ...
                    <div id="panel-existing-guest" class="guest-panel-existing guest-panel-hidden">
                            ...
                            <div class="selected-guest-header">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted" style="font-size: 0.75rem;" id="selectedGuestIdNumber">-
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div id="guestDirectoryPanel">
                            <div class="guest-search-wrapper">
                                ...
                                <input type="text" class="form-control form-control-sm guest-search-input"
                                    id="guestSearchInput" placeholder="Search guest name, NIK, or phone number..."
                                    autocomplete="off">
                            </div>
                            ...
                            <div class="guest-table-wrapper">
                                <table class="table guest-directory-table" id="guestDirectoryTable">
                                    ...
                                    <tbody id="guestTableBody">
                                        @forelse($guests as $g)
                                            <tr class="guest-row" data-guest-id="{{ $g->id }}"
                                                ...
                                                data-guest-profession="{{ $g->profession }}"
                                                data-guest-company="{{ $g->company }}"
                                                data-guest-member-card-no="{{ $g->member_card_no }}">
                                                <td><span
                                                        class="guest-table-avatar">{{ strtoupper(substr($g->name, 0, 1)) }}{{ strtoupper(substr(explode(' ', $g->name)[1] ?? '', 0, 1)) }}</span>
                                                </td>
                                                ...
                                                <td class="text-end">
                                                    <button type="button"
                                                        class="btn btn-outline-primary btn-select-guest"
                                                        onclick="selectGuestFromRow(this.closest('tr'))">
                            ...
                            <div class="guest-table-count">
                                ...
                                <span id="guestFilterInfo" style="display:none;"> — showing <span
                                        id="guestFilterCount">0</span> result(s)</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="panel-new-guest" class="guest-panel-new">
                        <div class="row g-3">
                            <div class="col-12 col-md-6">
                                <label for="name" class="form-label small fw-bold">Full Name <span
                            ...
                            <div class="col-12 col-md-6">
                                <label for="id_number" class="form-label small fw-bold">NIK / Passport No. <span
                                        class="text-danger">*</span></label>
                                <input type="text" name="id_number" id="id_number" value="{{ old('id_number') }}"
                                    ...
                                    placeholder="For identification card verification">
                                @error('id_number')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="birth_date" class="form-label small fw-bold">Birth Date <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="gender" class="form-label small fw-bold">Gender <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="country" class="form-label small fw-bold">Country / Nationality <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="phone" class="form-label small fw-bold">Phone Number <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="address" id="address" class="form-control form-control-sm @error('address') is-invalid @enderror"
                                    rows="2" placeholder="Full address details...">{{ old('address') }}</textarea>
                                @error('address')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="vehicle_no" id="vehicle_no" value="{{ old('vehicle_no') }}"
                                    class="form-control form-control-sm @error('vehicle_no') is-invalid @enderror"
                                    placeholder="e.g. B 1234 CD (Optional)">
                                @error('vehicle_no')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <!-- Additional Registration Card Fields (Always visible/editable) -->
                    <div class="border-top pt-3 mt-3">
                        <h5 class="h6 mb-3 text-secondary fw-semibold"><i class="bi bi-file-earmark-person"></i>
                            Registration Card Info</h5>
                        <div class="row g-3">
                            <div class="col-12 col-md-4">
                                <label for="profession" class="form-label small fw-bold">Profession / Pekerjaan</label>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="company" class="form-label small fw-bold">Company / Perusahaan</label>
                            ...
                            <div class="col-12 col-md-4">
                                <label for="member_card_no" class="form-label small fw-bold">Member Card No.</label>
                                <input type="text" name="member_card_no" id="member_card_no"
                                    value="{{ old('member_card_no') }}"
                                    class="form-control form-control-sm @error('member_card_no') is-invalid @enderror"
                                    placeholder="e.g. MB-12345 (Optional)">
            ...
            <div class="col-12 col-lg-5">
                <div class="panel-premium shadow-sm h-100 d-flex flex-column justify-content-between p-4">
                    <div>
                        <div class="panel-header border-bottom pb-2 mb-3">
                            <h2 class="h5 mb-0 section-title"><i
                                    class="bi bi-door-closed-fill text-primary"></i><span>Room & Dates</span></h2>
                        </div>

                        <div class="row g-3">
                            <div class="col-12">
                                <label for="room_id" class="form-label small fw-bold">Select Room <span
                                        class="text-danger">*</span></label>
                                <select name="room_id" id="room_id"
                                    ...
                                    @foreach ($rooms as $room)
                                        <option value="{{ $room->id }}"
                                            ...
                                            {{ old('room_id') == $room->id || (isset($preselectedRoom) && $preselectedRoom->id == $room->id) ? 'selected' : '' }}>
                                            Room {{ $room->room_number }} - {{ $room->roomType->name }}
                                            (Rp{{ number_format($room->roomType->price_per_night, 0, ',', '.') }}/N)
                                        </option>
                                    @endforeach
                                </select>
                                @error('room_id')
                            ...
                            <div class="col-12 col-md-4">
                                <label for="check_in_date" class="form-label small fw-bold">Check-in Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" name="check_in_date" id="check_in_date"
                                    value="{{ old('check_in_date', today()->toDateString()) }}"
                                    class="form-control form-control-sm @error('check_in_date') is-invalid @enderror"
                                    required>
                                @error('check_in_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12 col-md-4">
                                <label for="check_in_time" class="form-label small fw-bold">Check-in Time <span
                                        class="text-danger">*</span></label>
                                <input type="time" name="check_in_time" id="check_in_time"
                                    value="{{ old('check_in_time', '14:00') }}"
                                    class="form-control form-control-sm @error('check_in_time') is-invalid @enderror"
                                    required>
                                @error('check_in_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12 col-md-4">
                                <label for="check_out_date" class="form-label small fw-bold">Check-out Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" name="check_out_date" id="check_out_date"
                                    value="{{ old('check_out_date', today()->addDay()->toDateString()) }}"
                                    class="form-control form-control-sm @error('check_out_date') is-invalid @enderror"
                                    required>
                            ...
                            <div class="col-12 col-md-6 mt-3">
                                <label for="number_of_guests" class="form-label small fw-bold">Number of Guests <span
                                        class="text-danger">*</span></label>
                                <input type="number" name="number_of_guests" id="number_of_guests"
                                    value="{{ old('number_of_guests', '1') }}" min="1"
                                    class="form-control form-control-sm @error('number_of_guests') is-invalid @enderror"
                                    required>
                            ...
                            <div class="col-12 col-md-6 mt-3">
                                <label for="safety_deposit_box" class="form-label small fw-bold">Safety Deposit Box
                                    No.</label>
                                <input type="text" name="safety_deposit_box" id="safety_deposit_box"
                                    value="{{ old('safety_deposit_box') }}"
                                    class="form-control form-control-sm @error('safety_deposit_box') is-invalid @enderror"
                                    placeholder="e.g. SDB-101 (Optional)">
                                @error('safety_deposit_box')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-3 rounded border text-body shadow-xs"
                            style="background-color: rgba(0, 0, 0, 0.02); border-color: var(--admin-border) !important;">
                            <h6 class="fw-bold mb-3 text-primary"><i class="bi bi-calculator"></i> Estimated Folio
                                Statement</h6>
                            ...
                            <div class="d-flex justify-content-between small mb-2 border-bottom pb-2 text-secondary"
                                style="border-bottom-color: var(--admin-border) !important;">
                    ...
                    <div class="pt-4 border-top mt-4 d-flex gap-2"
                        style="border-top-color: var(--admin-border) !important;">
                        <button type="submit" class="btn btn-primary btn-tactile btn-sm w-100"><i
                                class="bi bi-check-circle"></i> Save Reservation</button>
                        <a href="{{ route('fo.reservations.index') }}"
                            class="btn btn-outline-secondary btn-tactile btn-sm w-100">Cancel</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            function selectGuestFromRow(row) {
                ...
                const guest = {
                    ...
                    profession: data.guestProfession,
                    company: data.guestCompany,
                    member_card_no: data.guestMemberCardNo,
                };
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function setGuestMode(mode) {
                    ...
                    // Reset registration card inputs
                    document.getElementById('profession').value = '';
                    document.getElementById('company').value = '';
                    document.getElementById('member_card_no').value = '';
                    ...
                    if (mode === 'new') {
                        ...
                        document.querySelectorAll(
                            '#panel-new-guest input, #panel-new-guest select, #panel-new-guest textarea').forEach(
                            el => {
                ...
                document.querySelectorAll('#panel-new-guest input[required], #panel-new-guest select[required]')
                    .forEach(el => {
                        el.dataset.wasRequired = 'true';
                    });
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    guestRows.forEach(row => {
                        ...
                        const matches = name.includes(query) || idNumber.includes(query) || phone
                            .includes(query);
                ...
                window._selectGuest = function(guest) {
                    ...
                    document.getElementById('selectedGuestBirthDate').textContent = guest.birth_date ? formatDate(
                        guest.birth_date) : '-';
                    ...
                    // Prefill form inputs for update
                    document.getElementById('profession').value = guest.profession || '';
                    document.getElementById('company').value = guest.company || '';
                    document.getElementById('member_card_no').value = guest.member_card_no || '';
                ...
                function updateMinCheckoutDate() {
                    if (inDateInput.value) {
                        const inDate = new Date(inDateInput.value);
                        if (!isNaN(inDate.getTime())) {
                            const nextDate = new Date(inDate);
                            nextDate.setDate(nextDate.getDate() + 1);
                            const minCheckout = formatLocalDate(nextDate);
                            outDateInput.setAttribute('min', minCheckout);

                            // If current check_out_date is less than or equal to check_in_date, automatically update it to nextDate
                            const outDate = new Date(outDateInput.value);
                            if (!isNaN(outDate.getTime()) && outDate <= inDate) {
                                outDateInput.value = minCheckout;
                            }
                        }
                    }
                }
