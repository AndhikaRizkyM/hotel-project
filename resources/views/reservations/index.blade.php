...
<div class="page-heading">
  <div class="page-heading-copy">
    ...
    <div>
      ...
      <p class="text-muted mb-0">Track reservation histories, active stays, and handle check-in / check-out workflows.</p>
    </div>
  </div>
...
<div class="panel mb-4 shadow-sm">
  <form method="GET" action="{{ route('fo.reservations.index') }}" class="row g-3">
    <div class="col-12 col-md-5">
      ...
      <input type="text" name="search" id="search" value="{{ request('search') }}" class="form-control form-control-sm" placeholder="Search by Res No or Guest Name...">
    </div>
  ...
  <div class="table-responsive">
    <table class="table align-middle">
      ...
      <tbody>
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" style="font-size: 0.85rem;">
                  ...
                  @if($res->status === 'RSV')
                    ...
                    <li>
                      <form action="{{ route('fo.reservations.check-in', $res->id) }}" method="POST" onsubmit="return confirm('Confirm check-in for {{ $res->guest->name }}?')">
                        @csrf
                        <button type="submit" class="dropdown-item text-success"><i class="bi bi-check-circle"></i> Process Check-In</button>
                      </form>
                    </li>
                    <li>
                      <form action="{{ route('fo.reservations.cancel', $res->id) }}" method="POST" onsubmit="return confirm('Cancel this reservation?')">
                        @csrf
                        <button type="submit" class="dropdown-item text-danger"><i class="bi bi-x-circle"></i> Cancel Reservation</button>
                      </form>
                    </li>
                    <li>
                      <form action="{{ route('fo.reservations.no-show', $res->id) }}" method="POST" onsubmit="return confirm('Mark this reservation as No Show?')">
                        @csrf
                        <button type="submit" class="dropdown-item text-warning"><i class="bi bi-exclamation-circle"></i> Mark as No Show</button>
                      </form>
                    </li>
                  @endif
                  ...
                  @if($res->status === 'CI')
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item text-danger" href="{{ route('fo.reservations.show', $res->id) }}#checkout-section"><i class="bi bi-box-arrow-right"></i> Process Check-Out</a></li>
                  @endif

                  <li><hr class="dropdown-divider"></li>
                  <li><a class="dropdown-item" href="{{ route('fo.print-registration', $res->id) }}" target="_blank"><i class="bi bi-printer"></i> Print Reg Card</a></li>
                  @if($res->status === 'CI' || $res->status === 'CO')
                    <li><a class="dropdown-item" href="{{ route('fo.print-invoice', $res->id) }}" target="_blank"><i class="bi bi-printer"></i> Print Folio Invoice</a></li>
                  @endif
                </ul>
              </div>
            </td>
        ...
        @empty
          <tr>
            <td colspan="7" class="text-center text-muted py-4">No reservations found matching the filters.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
