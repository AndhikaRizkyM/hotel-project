...
<div class="page-heading">
  ...
  <div class="heading-actions d-flex gap-2">
    <div class="dropdown d-inline-block">
      <button class="btn btn-outline-secondary btn-sm btn-tactile dropdown-toggle" type="button" id="printRegDropdown" data-bs-toggle="dropdown" aria-expanded="false">
        <i class="bi bi-printer"></i> Reg Card
      </button>
      <ul class="dropdown-menu dropdown-menu-end shadow border-0" aria-labelledby="printRegDropdown" style="border-radius: 10px; font-size: 0.82rem;">
        <li>
          <a class="dropdown-item py-2" href="{{ route('fo.print-registration', $reservation->id) }}?template=classic" target="_blank">
            <i class="bi bi-file-text me-2 text-secondary"></i> Classic Version (Old)
          </a>
        </li>
        <li>
          <a class="dropdown-item py-2" href="{{ route('fo.print-registration', $reservation->id) }}?template=modern" target="_blank">
            <i class="bi bi-file-earmark-richtext me-2 text-primary"></i> Premium Version (New)
          </a>
        </li>
      </ul>
    </div>
    <div class="dropdown d-inline-block">
      <button class="btn btn-outline-primary btn-sm btn-tactile dropdown-toggle" type="button" id="printInvoiceDropdown" data-bs-toggle="dropdown" aria-expanded="false">
        <i class="bi bi-file-earmark-spreadsheet"></i> Print Invoice
      </button>
      <ul class="dropdown-menu dropdown-menu-end shadow border-0" aria-labelledby="printInvoiceDropdown" style="border-radius: 10px; font-size: 0.82rem;">
        <li>
          <a class="dropdown-item py-2" href="{{ route('fo.print-invoice', $reservation->id) }}?template=classic" target="_blank">
            <i class="bi bi-file-text me-2 text-secondary"></i> Classic Version (Old)
          </a>
        </li>
        <li>
          <a class="dropdown-item py-2" href="{{ route('fo.print-invoice', $reservation->id) }}?template=modern" target="_blank">
            <i class="bi bi-file-earmark-richtext me-2 text-primary"></i> Premium Version (New)
          </a>
        </li>
      </ul>
  ...
  <div class="col-12 col-xl-4">
    ...
    <div class="panel mb-4 shadow-sm border-0" style="border-radius: 14px; background: var(--admin-surface);">
      ...
      <table class="table table-sm table-borderless small mb-0 text-body">
        ...
        @if($reservation->guest->profession)
          <tr><td class="fw-bold text-muted">Profession:</td><td class="fw-semibold">{{ $reservation->guest->profession }}</td></tr>
        @endif
        @if($reservation->guest->company)
          <tr><td class="fw-bold text-muted">Company:</td><td class="fw-semibold">{{ $reservation->guest->company }}</td></tr>
        @endif
        @if($reservation->guest->member_card_no)
          <tr><td class="fw-bold text-muted">Member Card:</td><td class="fw-semibold">{{ $reservation->guest->member_card_no }}</td></tr>
        @endif
      </table>
    </div>
    ...
    <div class="panel mb-4 shadow-sm border-0" style="border-radius: 14px; background: var(--admin-surface);">
      ...
      <table class="table table-sm table-borderless small mb-0 text-body">
        ...
        <tr><td class="fw-bold text-muted">Check-in:</td><td><span class="badge badge-soft-secondary">{{ $reservation->check_in_date->format('d M Y') }}</span> @if($reservation->check_in_time) <span class="badge bg-light text-secondary border border-secondary-subtle ms-1 px-2 py-1"><i class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($reservation->check_in_time)->format('H:i') }}</span> @endif</td></tr>
        <tr><td class="fw-bold text-muted">Check-out:</td><td><span class="badge badge-soft-secondary">{{ $reservation->check_out_date->format('d M Y') }}</span> @if($reservation->check_out_time) <span class="badge bg-light text-secondary border border-secondary-subtle ms-1 px-2 py-1"><i class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($reservation->check_out_time)->format('H:i') }}</span> @endif</td></tr>
        <tr><td class="fw-bold text-muted">No. of Guests:</td><td class="fw-semibold">{{ $reservation->number_of_guests }} Guest(s)</td></tr>
        @if($reservation->safety_deposit_box)
          <tr><td class="fw-bold text-muted">Safety Deposit Box:</td><td class="fw-semibold">{{ $reservation->safety_deposit_box }}</td></tr>
        @endif
        <tr>
            ...
            <div class="panel border-0 shadow-sm panel-soft p-4" style="border-radius: 14px; border: 1px solid rgba(239, 68, 68, 0.1) !important;">
              ...
              <form action="{{ route('fo.reservations.check-out', $reservation->id) }}" method="POST">
                ...
                <div class="p-3 bg-light rounded border mb-3 text-body" style="border-radius: 10px !important;">
                  <h6 class="fw-bold mb-2 text-secondary small">Settlement & Charges Summary</h6>
                  <div class="d-flex justify-content-between small mb-1">
                    <span>Scheduled Check-Out:</span>
                    <strong class="text-body">{{ $reservation->check_out_date->format('d M Y') }} ({{ \Carbon\Carbon::parse($reservation->check_out_time ?? '12:00')->format('H:i') }})</strong>
                  </div>
                  <div class="d-flex justify-content-between small mb-1">
                    <span>Base Folio Balance:</span>
                    <strong class="text-body" id="checkout-base-outstanding">Rp{{ number_format($outstanding, 0, ',', '.') }}</strong>
                  </div>
                  <div class="d-flex justify-content-between small mb-1 text-danger d-none" id="checkout-late-row">
                    <span>Late Check-Out Charge (Estimated):</span>
                    <strong id="checkout-late-charge">Rp0</strong>
                  </div>
                  <hr class="my-2">
                  <div class="d-flex justify-content-between fw-bold">
                    <span id="checkout-total-outstanding-label">{{ $outstanding < -0.01 ? 'Refund Due to Guest:' : 'Total Outstanding Balance:' }}</span>
                    <strong class="{{ $outstanding > 0.01 ? 'text-danger' : 'text-success' }}" id="checkout-total-outstanding">
                      Rp{{ number_format(abs($outstanding), 0, ',', '.') }}
                    </strong>
                  </div>
                </div>
                ...
                <div class="row g-3">
                  <div class="col-12 col-md-4">
                    <label for="actual_check_out_time" class="form-label small fw-bold">Actual Check-Out Time</label>
                    <input type="time" name="actual_check_out_time" id="actual_check_out_time" value="{{ now()->format('H:i') }}" class="form-control form-control-sm" required style="border-radius: 8px;">
                  </div>
                  <div class="col-12 col-md-4">
                    <div id="payment-method-field" class="{{ $outstanding > 0.01 ? '' : 'd-none' }}">
                      <label for="checkout_payment_method" class="form-label small fw-bold">Payment Method</label>
                      <select name="payment_method" id="checkout_payment_method" class="form-select form-select-sm" {{ $outstanding > 0.01 ? 'required' : '' }} style="border-radius: 8px;">
                  ...
                  <div class="col-12 col-md-4 d-flex align-items-end">
                    <button type="submit" class="btn btn-danger btn-tactile btn-sm text-nowrap w-100 py-2" style="border-radius: 8px; height: 38px;">
...
<script>
  ...
  // Realtime Checkout Late Charge Calculator
  const actualCheckOutTimeInput = document.getElementById('actual_check_out_time');
  if (actualCheckOutTimeInput) {
    const originalOutstanding = parseFloat("{{ $outstanding }}") || 0;
    const scheduledTimeStr = "{{ $reservation->check_out_time ?? '12:00' }}";
    
    function recalculateCheckoutCharges() {
      const actualTimeVal = actualCheckOutTimeInput.value;
      if (!actualTimeVal) return;

      const [schHour, schMin] = scheduledTimeStr.split(':').map(Number);
      const [actHour, actMin] = actualTimeVal.split(':').map(Number);

      const schTotalMinutes = schHour * 60 + schMin;
      const actTotalMinutes = actHour * 60 + actMin;

      let lateCharge = 0;
      let hoursLate = 0;

      if (actTotalMinutes > schTotalMinutes) {
        const diffMinutes = actTotalMinutes - schTotalMinutes;
        hoursLate = Math.ceil(diffMinutes / 60);
        lateCharge = hoursLate * 100000;
      }

      const totalOutstanding = originalOutstanding + lateCharge;

      // Update late row visibility and value
      const lateRow = document.getElementById('checkout-late-row');
      const lateChargeEl = document.getElementById('checkout-late-charge');
      if (lateCharge > 0) {
        lateChargeEl.textContent = `Rp${lateCharge.toLocaleString('id-ID')} (${hoursLate} Hour(s) Late)`;
        lateRow.classList.remove('d-none');
      } else {
        lateRow.classList.add('d-none');
      }

      // Update total outstanding text and style
      const totalOutstandingEl = document.getElementById('checkout-total-outstanding');
      const outstandingLabelEl = document.getElementById('checkout-total-outstanding-label');
      
      if (totalOutstanding > 0.01) {
        totalOutstandingEl.textContent = `Rp${totalOutstanding.toLocaleString('id-ID')}`;
        totalOutstandingEl.className = 'text-danger';
        if (outstandingLabelEl) outstandingLabelEl.textContent = 'Total Outstanding Balance:';
        
        // Show payment method
        document.getElementById('payment-method-field').classList.remove('d-none');
        document.getElementById('checkout_payment_method').setAttribute('required', '');
      } else if (totalOutstanding < -0.01) {
        totalOutstandingEl.textContent = `Rp${Math.abs(totalOutstanding).toLocaleString('id-ID')}`;
        totalOutstandingEl.className = 'text-success';
        if (outstandingLabelEl) outstandingLabelEl.textContent = 'Refund Due to Guest:';
        
        // Hide payment method (it's a refund)
        document.getElementById('payment-method-field').classList.add('d-none');
        document.getElementById('checkout_payment_method').removeAttribute('required');
        document.getElementById('checkout_payment_method').value = '';
      } else {
        totalOutstandingEl.textContent = `Rp0`;
        totalOutstandingEl.className = 'text-success';
        if (outstandingLabelEl) outstandingLabelEl.textContent = 'Total Outstanding Balance:';
    ...
    actualCheckOutTimeInput.addEventListener('input', recalculateCheckoutCharges);
    recalculateCheckoutCharges();
  }
</script>
@endpush
