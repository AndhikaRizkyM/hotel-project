...
<div class="panel-premium mb-4 shadow-sm">
  <form method="GET" action="{{ route('fo.availability') }}" class="row g-3">
    ...
    <div class="col-12 col-md-3 d-flex align-items-end">
      <button type="submit" class="btn btn-primary btn-tactile btn-sm me-2 w-100"><i class="bi bi-funnel"></i> Filter</button>
      <a href="{{ route('fo.availability') }}" class="btn btn-outline-secondary btn-tactile btn-sm w-100"><i class="bi bi-arrow-clockwise"></i> Reset</a>
    </div>
  </form>
</div>
...
<div class="panel-premium shadow-sm">
  ...
  @forelse($floors as $floor => $floorRooms)
    <div class="mb-4">
      <h6 class="text-uppercase fw-bold text-muted border-bottom pb-2" style="font-size: 0.75rem; letter-spacing: 1px;"><i class="bi bi-layers-half text-secondary me-1"></i> Floor {{ $floor }}</h6>
            ...
            <div class="room-bento-card room-{{ $room->status }} btn-tactile">
              <div class="d-flex flex-column justify-content-between h-100">
                <div>
                  ...
                  <p class="mb-2 text-body font-monospace fw-semibold" style="font-size: 0.75rem; opacity: 0.9;">Rp{{ number_format($room->roomType->price_per_night, 0, ',', '.') }}/N</p>
                </div>
                
                <div class="d-flex align-items-center justify-content-between gap-1 mt-auto pt-2 border-top">
                  <button type="button" class="btn-room-info border-0 shadow-sm" title="View Room Details"
                  ...
                  <div class="d-inline-flex gap-1">
                    @if($room->status === 'A')
                      <a href="{{ route('fo.reservations.create', ['room_id' => $room->id]) }}" class="btn btn-success btn-tactile btn-xs py-1 px-2" style="font-size: 0.7rem;"><i class="bi bi-bookmark-plus"></i> Book</a>
                    @elseif($room->status === 'R')
                      @php 
                        $res = $room->reservations()->where('status', 'RSV')->first();
                      @endphp
                      @if($res)
                        <a href="{{ route('fo.reservations.show', $res->id) }}" class="btn btn-info text-white btn-tactile btn-xs py-1 px-2" style="font-size: 0.7rem;"><i class="bi bi-eye"></i> View</a>
                      @endif
                    @elseif($room->status === 'O')
                      ...
                      @endphp
                      @if($res)
                        <a href="{{ route('fo.reservations.show', $res->id) }}" class="btn btn-primary btn-tactile btn-xs py-1 px-2" style="font-size: 0.7rem;"><i class="bi bi-receipt"></i> Folio</a>
                      @endif
                    @else
                      <span class="text-muted small text-center py-1" style="font-size: 0.65rem; white-space: nowrap;">Service Mode</span>
                    @endif
                  </div>
                </div>
              </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-header align-items-start">
        <div class="d-flex justify-content-between align-items-start w-100" style="position: relative; z-index: 2;">
          <div>
      ...
      <div class="modal-footer justify-content-between">
        <button type="button" class="btn btn-light btn-tactile btn-sm" data-bs-dismiss="modal"><i class="bi bi-x-lg"></i> Close</button>
        <a href="#" class="btn btn-primary btn-tactile btn-sm" id="modal-book-btn" style="display: none;">
