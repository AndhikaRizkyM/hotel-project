...
<div class="page-heading">
  ...
  <div class="heading-actions">
    <a href="{{ route('reports.export-excel', ['start_date' => request('start_date'), 'end_date' => request('end_date')]) }}" class="btn btn-success btn-tactile btn-sm"><i class="bi bi-file-earmark-excel"></i> Export to Excel (.xls)</a>
  </div>
</div>
...
<div class="panel-premium mb-4 shadow-sm">
  <form method="GET" action="{{ route('reports.index') }}" class="row g-3 align-items-end">
    ...
    <div class="col-12 col-md-4 d-flex">
      <button type="submit" class="btn btn-primary btn-tactile btn-sm me-2 w-100"><i class="bi bi-funnel"></i> Generate Report</button>
      <a href="{{ route('reports.index') }}" class="btn btn-outline-secondary btn-tactile btn-sm w-100"><i class="bi bi-arrow-clockwise"></i> Reset</a>
    </div>
  </form>
</div>
...
<section class="row g-3 mb-4">
  <div class="col-12 col-sm-6 col-md-3">
    <div class="metric-card metric-primary shadow-sm h-100">
      ...
      <div class="metric-value" style="font-size: 1.65rem;">{{ $occupiedRooms }} / {{ $totalRooms }} Rms</div>
      <div class="metric-meta text-muted">Occupancy rate: <strong class="text-primary">{{ number_format($occupancyRate, 1) }}%</strong></div>
    </div>
  </div>
  <div class="col-12 col-sm-6 col-md-3">
    <div class="metric-card metric-success shadow-sm h-100">
      ...
      <div class="metric-value" style="font-size: 1.65rem;">Rp{{ number_format($totalRevenue, 0, ',', '.') }}</div>
  ...
  <div class="col-12 col-sm-6 col-md-3">
    <div class="metric-card {{ $netCashFlow >= 0 ? 'metric-success' : 'metric-danger' }} shadow-sm h-100">
      ...
      <div class="metric-value" style="font-size: 1.65rem; color: var(--{{ $netCashFlow >= 0 ? 'admin-success' : 'admin-danger' }}) !important;">Rp{{ number_format($netCashFlow, 0, ',', '.') }}</div>
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle table-sm small mb-0">
          ...
          <tbody>
            ...
            @endforeach
            <tr class="table-light fw-bold border-top" style="border-top: 2px solid var(--admin-border) !important; background: rgba(0,0,0,0.02);">
              <td>TOTAL REVENUE:</td>
        ...
        <table class="table table-hover table-sm small align-middle mb-0">
          ...
          <tbody>
            @forelse($recentTransactions as $tx)
              <tr>
                <td class="text-muted">{{ \Carbon\Carbon::parse($tx->transaction_date)->format('d/m H:i') }}</td>
                ...
                <td>
                  <span class="badge {{ $tx->type === 'payment' ? 'badge-soft-success' : 'badge-soft-danger' }}">{{ ucfirst($tx->type) }}</span>
                </td>
