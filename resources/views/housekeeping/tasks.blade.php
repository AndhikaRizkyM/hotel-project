                ...
                <form method="GET" action="{{ route('hk.tasks') }}" class="row g-2">
                    <div class="col-12 col-sm-4">
                        <select name="status" class="form-select form-select-sm">
                            ...
                            <option value="pending" {{ request('status') === 'pending' ? 'selected' : '' }}>Pending Cleaning
                            </option>
                            <option value="cleaning" {{ request('status') === 'cleaning' ? 'selected' : '' }}>In Progress
                            </option>
                            <option value="ready_for_inspection"
                                {{ request('status') === 'ready_for_inspection' ? 'selected' : '' }}>Ready for Inspection
                            </option>
                    ...
                    <div class="col-12 col-sm-4 d-flex">
                        <button type="submit" class="btn btn-primary btn-sm me-2 w-100"><i class="bi bi-funnel"></i>
                            Filter</button>
                        <a href="{{ route('hk.tasks') }}" class="btn btn-outline-secondary btn-sm w-100"><i
        ...
        <div class="table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    @forelse($tasks as $task)
                        <tr>
                            ...
                            <td>
                                ...
                                @endphp
                                <span
                                    class="badge bg-{{ $colors[$task->status] ?? 'secondary' }}">{{ $labels[$task->status] ?? $task->status }}</span>
                            </td>
                            <td>
                                @if ($task->assignedTo)
                            ...
                            <td>
                                @if ($task->start_time)
                                    <span class="d-block small text-muted"><strong>Started:</strong>
                                        {{ $task->start_time->format('d/m H:i') }}</span>
                                @endif
                                @if ($task->end_time)
                                    <span class="d-block small text-muted"><strong>Completed:</strong>
                                        {{ $task->end_time->format('d/m H:i') }}</span>
                                @endif
                            </td>
                            <td>
                                @if ($task->status === 'pending')
                                    <button type="button" class="btn btn-success btn-xs" data-bs-toggle="modal"
                                        data-bs-target="#startTaskModal-{{ $task->id }}"><i
                                            class="bi bi-play-circle"></i> Start Cleaning</button>
                                @elseif($task->status === 'cleaning')
                                    <form action="{{ route('hk.tasks.complete', $task->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-primary btn-xs"><i
                                                class="bi bi-check-circle"></i> Finish Cleaning</button>
                                    </form>
                                @elseif($task->status === 'ready_for_inspection')
                                    <a href="{{ route('hk.inspections.index') }}" class="btn btn-info btn-xs text-white"><i
                                            class="bi bi-eye"></i> Log Quality Inspection</a>
                                @else
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title fw-bold"><i class="bi bi-play-fill text-success"></i>
                                                Start Cleaning Room {{ $task->room->room_number }}</h5>
                                        ...
                                        <form action="{{ route('hk.tasks.start', $task->id) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    <label for="attendant-{{ $task->id }}"
                                                        class="form-label small fw-bold">Select Housekeeping
                                                        Attendant</label>
                                                    <select name="assigned_to_user_id" id="attendant-{{ $task->id }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">No active housekeeping cleaning tasks
                                found.</td>
                        </tr>
                    @endforelse
                </tbody>
