    ...
    <div class="panel p-4">
      <form action="{{ route('master.rooms.store') }}" method="POST" class="needs-validation" novalidate>
        ...
        <div class="mb-3">
          ...
          <input class="form-control" name="room_number" id="roomNumber" type="text" placeholder="e.g. 101, 204" required>
          <div class="invalid-feedback">Room number is required.</div>
        </div>

        <div class="mb-3">
          <label class="form-label" for="roomFloor">Floor</label>
          <input class="form-control" name="floor" id="roomFloor" type="number" min="1" placeholder="e.g. 1, 2, 3" required>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="room_type_id" id="roomTypeId" required>
            ...
            @foreach($roomTypes as $type)
              <option value="{{ $type->id }}">{{ $type->name }} (Rp{{ number_format($type->price_per_night, 0, ',', '.') }}/night)</option>
            @endforeach
          </select>
