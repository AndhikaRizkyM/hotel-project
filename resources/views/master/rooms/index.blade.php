...
@section('content')
    <div class="page-heading">
        ...
        <div class="heading-actions">
            <a href="{{ route('master.rooms.create') }}" class="btn btn-primary btn-sm px-4 py-2"><i
                    class="bi bi-plus-circle"></i> Create Room</a>
        </div>
    </div>
                ...
                <tbody>
                    @foreach ($rooms as $room)
                        <tr>
                            ...
                            <td class="text-end">
                                <a href="{{ route('master.rooms.edit', $room->id) }}" class="btn btn-light btn-sm"><i
                                        class="bi bi-pencil"></i> Edit</a>
                                <form action="{{ route('master.rooms.toggle', $room->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    <button class="btn btn-outline-{{ $room->is_active ? 'danger' : 'success' }} btn-sm"
                                        type="submit">
