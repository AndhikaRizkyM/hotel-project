...
<div class="page-heading">
  ...
  <div class="heading-actions">
    <a href="{{ route('master.rooms.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left"></i> Back</a>
  </div>
</div>
...
<div class="row mt-3">
  <div class="col-12 col-md-8 col-lg-6">
    <div class="panel p-4">
      <form action="{{ route('master.rooms.update', $room->id) }}" method="POST" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
        
        <div class="mb-3">
          <label class="form-label" for="roomNumber">Room Number</label>
          <input class="form-control" name="room_number" id="roomNumber" type="text" value="{{ old('room_number', $room->room_number) }}" required>
          <div class="invalid-feedback">Room number is required.</div>
        </div>

        <div class="mb-3">
          <label class="form-label" for="roomFloor">Floor</label>
          <input class="form-control" name="floor" id="roomFloor" type="number" value="{{ old('floor', $room->floor) }}" required>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="status" id="roomStatus" required>
            ...
            <option value="O" {{ old('status', $room->status) === 'O' ? 'selected' : '' }}>Occupied</option>
            <option value="D" {{ old('status', $room->status) === 'D' ? 'selected' : '' }}>Dirty</option>
            <option value="C" {{ old('status', $room->status) === 'C' ? 'selected' : '' }}>Cleaning</option>
            <option value="M" {{ old('status', $room->status) === 'M' ? 'selected' : '' }}>Maintenance</option>
            <option value="R" {{ old('status', $room->status) === 'R' ? 'selected' : '' }}>Reserved</option>
            <option value="B" {{ old('status', $room->status) === 'B' ? 'selected' : '' }}>Blocked</option>
          </select>
