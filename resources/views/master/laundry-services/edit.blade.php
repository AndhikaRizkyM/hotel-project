...
@block('title', 'Edit Laundry Service')
...
<div class="page-heading">
  ...
  <div class="heading-actions">
    <a href="{{ route('master.laundry-services.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left"></i> Back</a>
  </div>
</div>
...
<div class="row mt-3">
  <div class="col-12 col-md-8 col-lg-6">
    <div class="panel p-4">
      <form action="{{ route('master.laundry-services.update', $service->id) }}" method="POST" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
        
        <div class="mb-3">
          <label class="form-label" for="serviceName">Service Name</label>
          <input class="form-control" name="name" id="serviceName" type="text" value="{{ old('name', $service->name) }}" required>
          <div class="invalid-feedback">Service name is required.</div>
        </div>

        <div class="mb-3">
          <label class="form-label" for="servicePrice">Price (Rp)</label>
          <input class="form-control" name="price" id="servicePrice" type="number" value="{{ old('price', $service->price) }}" required>
