    ...
    <div class="panel p-4">
      ...
      <form action="{{ route('master.users.store') }}" method="POST" class="needs-validation" novalidate>
        ...
        <div class="mb-3">
          ...
          <input class="form-control" name="name" id="userName" type="text" value="{{ old('name') }}" required>
          <div class="invalid-feedback">Please enter name.</div>
        </div>

        <div class="mb-3">
          <label class="form-label" for="userEmail">Email address</label>
          <input class="form-control" name="email" id="userEmail" type="email" value="{{ old('email') }}" required>
